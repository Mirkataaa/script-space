...
export const saveExec = mutation({
  args: {
    language: v.string(),
    code: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    if (!user?.isPremium && args.language !== "javascript") {
      throw new ConvexError(
        "Premium subscription is required to use this language!"
      );
    }
...
export const getUserStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    // get all starred snippet details
    ...
    const favLanguage = languages.length
